<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight">
            🔧 Añadir Módulo a: <span class="bg-gradient-to-r from-[#9c0000] to-[#ff4b65] bg-clip-text text-transparent">{{ $vivero->nombre }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white/90 backdrop-filter backdrop-blur-lg overflow-hidden rounded-2xl p-8" style="box-shadow: 0 8px 32px rgba(156, 0, 0, 0.08);">
                <h2 class="text-3xl font-bold bg-gradient-to-r from-[#9c0000] to-[#ff4b65] bg-clip-text text-transparent mb-8">Detalles del Nuevo Módulo</h2>
                ...
                <form method="POST" action="{{ route('admin.viveros.modulos.store', $vivero) }}">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input id="codigo_identificador" name="codigo_identificador" type="text" value="{{ old('codigo_identificador') }}" required autofocus class="block w-full px-4 py-2 bg-white border-[#e0e0e0] text-[#1a1a1a] focus:border-[#ff4b65] focus:ring-2 focus:ring-[#ffdef0] rounded-lg shadow-sm transition">
                        </div>

                        <div>
                            <label for="capacidad" class="block font-semibold text-sm text-[#1a1a1a] mb-2">🌱 Capacidad (Nº de plantas)</label>
                            <input id="capacidad" name="capacidad" type="number" value="{{ old('capacidad') }}" required class="block w-full px-4 py-2 bg-white border-[#e0e0e0] text-[#1a1a1a] focus:border-[#ff4b65] focus:ring-2 focus:ring-[#ffdef0] rounded-lg shadow-sm transition">
                        </div>
